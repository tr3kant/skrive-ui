...
class YouTube extends Shortcode {
	...
	public static function get_shortcode_ui_args() {
		return array(
			'label'          => esc_html__( 'YouTube', 'skrive-ui' ),
			// 'listItemImage'  => '<img src="' . esc_url( SHORTCAKE_BAKERY_URL_ROOT . 'assets/images/svg/icon-youtube.svg' ) . '" />',
			'attrs'          => array(
				array(
					'label'        => esc_html__( 'URL', 'skrive-ui' ),
					'attr'         => 'url',
					'type'         => 'text',
					'description'  => esc_html__( 'Full YouTube URL', 'skrive-ui' ),
				),
			),
		);
	}
	...
	public static function callback( $attrs, $content = '' ) {
		...
		$embed_url = apply_filters( 'skrive_ui_youtube_embed_url', $embed_url, $attrs );
		return sprintf( '<div class="video-container"><iframe class="youtube-embed" width="640" height="360" src="%s" frameborder="0"></iframe></div>', esc_url( $embed_url ) );
	}
